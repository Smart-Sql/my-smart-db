...
class ScheduleFutureImpl<R> implements SchedulerFuture<R> {
     ...
     */
    ScheduleFutureImpl(String name, Scheduler sched, GridKernalContext ctx, String pat) {
        assert sched != null;
        assert ctx != null;
        assert pat != null;
    ...
    void schedule(Callable<R> task) {
        ...
        ((IgniteScheduleProcessor)ctx.schedule()).onScheduled(this.name, this);
    ...
    void deschedule() {
        if (descheduled.compareAndSet(false, true)) {
            ...
            ((IgniteScheduleProcessor)ctx.schedule()).onDescheduled(this.name);
        }
    }
